## 🛒 Ecommerce Sales Dashboard – Power BI
An interactive Power BI dashboard built to analyze ecommerce sales performance across states, categories, and customer segments.

## 📊 Dashboard Overview
The dashboard tracks key metrics including total sales amount (438K), profit (37K), quantity sold (5,615), and average order value (121K) — with quarterly and state-level filters for dynamic exploration.

## 📌 Key Visuals

Profit by Month – Bar chart showing monthly profit trends, highlighting loss months (May, June, August)
Sales by State – Horizontal bar chart comparing revenue across Maharashtra, Madhya Pradesh, and Uttar Pradesh
Quantity by Category – Donut chart showing Clothing (63%), Electronics (21%), and Furniture (17%)
Payment Mode Breakdown – COD leads at 44%, followed by UPI (21%), Debit Card (13%), Credit Card (12%), and EMI (10%)
Top Customers – Bar chart of highest-value customers: Harivansh, Madhav, Madan Mohan, Shiva
Profit by Sub-Category – Printers and Bookcases are the top profitable sub-categories

## 🔧 Tools Used
Power BI Desktop
Microsoft Excel (Data Source)

###  Dashboard Creation

* Built an interactive **Power BI dashboard** including:

  * KPI Cards
  * Trend charts
  * Filters & slicers
  * Category-wise performance visuals

## 📊 Dashboard Preview

![Sales Dashboard](Sales_Dashboard.png)
The dashboard provides a complete overview of ecommerce performance,
tracking ₹438K in total sales, ₹37K profit, 5,615 units sold, and
₹121K average order value — with breakdowns by state, category,
customer, and payment mode.

![Sales Trends](sales_trends.png)
The profit trend chart reveals strong performance in Q1 (Jan–Apr),
followed by a dip into losses during May–August, with a recovery
peaking in November — offering clear visibility into seasonal
patterns and low-performing periods.

## 💡 Key Insights

Clothing dominates sales volume at 63% of total quantity
Q1 (Jan–Mar) shows the strongest profit months
COD is the most preferred payment method
Printers generate the highest sub-category profit
